"""
Author: Silas Udofia
Date: 2024-08-02
GitHub: https://github.com/Silas-U/RoboKpy/tree/main

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at: http://www.apache.org/licenses/LICENSE-2.0
"""

from robokpy import Init_Model
from Models.Model import DHModel
import numpy as np

model = DHModel.get_model('Cobra600')
rb = Init_Model(model, robot_name='Cobra600', twist_in_rads=True, use_jnt_lim=True, plt_model=True)

rb.model.set_joint_limits(
    {
        "min": { "j1": -np.pi, "j2": -np.pi/2, "j3": 0.04, "j4": -np.pi },
        "max": { "j1": np.pi,"j2": np.pi/2, "j3": 0.06, "j4": np.pi }
    }
)

# Joint Control Mode
j_poses = [
    [90, 0, 0.05, 0],
    [0, 90, 0.05, 0],
    [-90, 90, 0.05, 0],
    [0, 0, 0.05,  0],
    [0, 0, 0.2,  0],
    [0, 0, 0.2,  90],
    [90, 0, 0.05, 0],
]

rb.traj.traj_type('qu')
wp = rb.traj.joint_control(j_poses, n_samples=20, rads=False)
rb.plotter.plot_traj(plot_type='xyz', selected_plot='all', projection='2d')

rb.mviz.scale_viz(0.5, 0.05)
rb.mviz.plot(wp, show_path=True, show_via_points=True, repeat=True)
